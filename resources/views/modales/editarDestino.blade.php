<!-- Main modal -->
<div id="editarDestino{{$destino->id}}" data-modal-backdrop="static" tabindex="-1" aria-hidden="true" class=" hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
    <div class="relative p-4 w-full max-w-md max-h-full">
        <!-- Modal content -->
        <div class="relative bg-white rounded-lg shadow px-5 pb-4 dark:bg-gray-700  border border-t-orange-500 border-t-4">
            <!-- Modal header -->
            <div class="flex items-center justify-between md:p-5 border-b rounded-t dark:border-gray-600">
                <h3 class="text-lg font-semibold text-gray-900 dark:text-white">
                    Editar Destino
                </h3>
                <button type="button" class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white" data-modal-toggle="editarDestino{{$destino->id}}">
            ...
            <!-- Modal body -->
            <form class="" method="POST" action="{{route('editarDestino')}}">
                @csrf
                    ...
                    <div class="col-span-2 sm:col-span-2">
                        ...
                        <select id="pais" name="pais" class=" bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500">
                            @foreach($paises as $pais)
                                <option value="{{$pais->id}}" {{$pais->nombre == $destino->pais ? 'selected' : ''}}>{{$pais->nombre}}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-span-2 sm:col-span-2">
                        ...
                        <select id="ciudad" name="ciudad" class=" bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500">
                            @foreach($ciudades as $ciudad)
                                <option value="{{$ciudad->id}}" {{$ciudad->nombre == $destino->ciudad ? 'selected' : ''}}>{{$ciudad->nombre}}</option>
                    ...
                    <div class="col-span-2 sm:col-span-2">
                        ...
                        <select id="aerolinea" name="aerolinea" class=" bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500">
                            @foreach($aerolineas as $aerolinea)
                                <option value="{{$aerolinea->id}}" {{$aerolinea->nombre == $destino->aerolinea ? 'selected' : ''}}>{{$aerolinea->nombre}}</option>
                            @endforeach
                        </select>
